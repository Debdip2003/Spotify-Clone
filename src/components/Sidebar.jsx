import React from 'react'
import {assets} from '../assets/assets'
import Player from './Player'
import { useNavigate } from 'react-router-dom'

const Sidebar = () => {
    const navigate=useNavigate()
    return (
        <div className='w-[25%] h-screen p-2 flex-col gap-2 text-white hidden lg:flex'>
            <div className='bg-[#121212] h-[15%] rounded flex flex-col justify-around'>
                <div onClick={()=>navigate('/')} className='flex items-center pl-8  pt-2 gap-3 cursor-pointer'>
                    <img src={assets.home_icon} alt='' className='w-6'/>
                    <p className='font-bold'>Home</p>
                </div>
                <div className='flex items-center pl-8 gap-3 mt-4 pb-2 cursor-pointer'>
                    <img src={assets.search_icon} alt='' className='w-6'/>
                    <p className='font-bold' onClick={()=>navigate('/search')}>Search</p>
                </div>
            </div>
            <div className='bg-[#121212] h-[85%]'>
                <div className='p-4 flex items-center justify-between'>
                    <div className='flex items-center gap-3'>
                        <img src={assets.stack_icon} alt='' className='w-8'/>
                        <p className='font-semibold'>Your Library</p>
                    </div>
                    <div className='flex items-center gap-3'>
                        <img src={assets.arrow_icon} alt='' className='w-5'/>
                        <img src={assets.plus_icon} alt='' className='w-5'/>
                    </div>
                </div>
                <div className='p-4 bg-[#242424] m-2 rounded font-semibold flex flex-col items-start justify-start pl-4 gap-1'>
                    <h1>Create your first playlist</h1>
                    <p className='font-light'>It's easy we will help you</p>
                    <button className='px-4 py-1.5 bg-white text-[15px] text-black rounded-full mt-4 hover:text-white hover:bg-black'>Create Playlist</button>
                </div>
                <div className='p-4 bg-[#242424] m-2 rounded font-semibold flex flex-col items-start mt-4 justify-start pl-4 gap-1'>
                    <h1>Let's find some podcast to follow</h1>
                    <p className='font-light'>We'll keep you update on new episodes</p>
                    <button className='px-4 py-1.5 bg-white text-[15px] text-black rounded-full mt-4 hover:text-white hover:bg-black'>Browse podcasts</button>
                </div>
                <div className='p-4 bg-[#242424] m-2 rounded font-semibold flex flex-col items-start mt-4 justify-start pl-4 gap-1'>
                    <h1>Let's find some albums</h1>
                    <p className='font-light'>We'll keep yoou update on new albums</p>
                    <button className='px-4 py-1.5 bg-white text-[15px] text-black rounded-full mt-4 hover:text-white hover:bg-black'>Browse podcasts</button>
                </div>
            </div>
            <Player/>
        </div>
    )
}

export default Sidebar
